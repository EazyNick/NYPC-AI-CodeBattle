...
// 샘플링 관련 전역 설정
const MAX_COMBINATIONS: usize = 100; // 최대 조합 수 (C(10,5)=252, C(12,5)=792)
...
impl Game {
    ...
    fn check_my_yacht_completion(&mut self) {
        // 내가 YACHT 규칙을 사용했는지 확인
        if let Some(score) = self.my_state.rule_score[DiceRule::Yacht as usize] {
            if score > 0 {
    ...
    fn calculate_bid(&self, dice_a: &[i32], dice_b: &[i32]) -> Bid {
        // 간단한 그룹 선택: 중복이 더 많은 그룹 선택
        let max_count_a = self.get_max_duplicate_count(dice_a);
        let max_count_b = self.get_max_duplicate_count(dice_b);
        
        let group = if max_count_a >= max_count_b { 'A' } else { 'B' };
        
        // 간단한 입찰 점수 계산
        let amount = match self.current_round {
            1 => {
                // 첫 번째 턴: 중복 개수에 따라 간단한 점수
                let max_count = max_count_a.max(max_count_b);
                match max_count {
                    3 => 1001,
                    4 => 2001,
                    _ => 1,
            ...
            2 => {
                // 두 번째 턴: 상대방 첫 턴 입찰에 따라
                let first_round_bid = self.opponent_bids.get(&1).unwrap_or(&0);
                if *first_round_bid > 1 { 101 } else { 0 }
            },
            3 => {
                // 세 번째 턴: 상대방 두 번째 턴 홀짝에 따라
                let second_round_bid = self.opponent_bids.get(&2).unwrap_or(&0);
                if *second_round_bid % 2 == 1 { 2 } else { 0 }
            },
            _ => {
                // 4번째 턴 이후: 상대방 두 번째 턴 홀짝에 따라
                let second_round_bid = self.opponent_bids.get(&2).unwrap_or(&0);
                if *second_round_bid % 2 == 1 { 2 } else { 0 }
            }
        };
    ...
    fn calculate_put(&self) -> DicePut {
        ...
        if self.current_round == 13 {
            return self.calculate_13th_round_put();
        }
        
        // 11번째 턴 이후: 간단한 조합 사용 (시간 최적화)
        if self.current_round >= 11 {
            return self.calculate_simple_put();
        }
        
        // 1-10번째 턴: 규칙별 최적 주사위 조합 찾기
        let unused_rules: Vec<usize> = self.my_state.rule_score
            .iter()
            .enumerate()
            .filter_map(|(i, &score)| if score.is_none() { Some(i) } else { None })
        ...
        // 각 규칙별로 최적의 주사위 조합 찾기
        for &rule_index in &unused_rules {
            if let Some(rule) = DiceRule::from_usize(rule_index) {
                let (optimal_dice, score) = self.find_optimal_dice_for_rule(rule);
                if score > best_score {
                    best_score = score;
                    best_rule = rule_index;
                    best_dice = optimal_dice;
                }
            }
        }
    ...
    fn calculate_simple_put(&self) -> DicePut {
        let unused_rules: Vec<usize> = self.my_state.rule_score
            .iter()
            .enumerate()
            .filter_map(|(i, &score)| if score.is_none() { Some(i) } else { None })
        ...
        let rule_index = unused_rules.first().unwrap_or(&0);
        let dice: [i32; 5] = if self.my_state.dice.len() >= 5 {
            self.my_state.dice[..5].try_into().unwrap()
        } else {
            [0; 5]
        };
        
        DicePut {
            rule: DiceRule::from_usize(*rule_index).unwrap(),
            dice,
        }
    }
    
    /// 12번째 라운드에서 10개 주사위를 5개씩 나누어 최적 조합 찾기
    fn calculate_final_round_put(&self) -> DicePut {
        let my_dice = &self.my_state.dice;
        
        // 12번째 턴: 10개 주사위에서 최적 조합 찾기
        let unused_rules: Vec<usize> = self.my_state.rule_score
            .iter()
            .enumerate()
            .filter_map(|(i, &score)| if score.is_none() { Some(i) } else { None })
        ...
        // 10개 주사위에서 가능한 모든 5개 조합을 시도 (샘플링 적용)
        if my_dice.len() >= 10 {
            let combinations = self.generate_all_5_combinations(my_dice);
            
            // 샘플링: 조합 수가 많으면 제한
            let combinations_to_try = if combinations.len() > MAX_COMBINATIONS {
                &combinations[..MAX_COMBINATIONS]
            } else {
                &combinations
            };
            
            for &dice in combinations_to_try {
                for &rule_index in &unused_rules {
                    if let Some(rule) = DiceRule::from_usize(rule_index) {
                        let score = self.calculate_rule_potential_score(&dice, rule);
                        if score > best_score {
    ...
    /// 규칙별 최적 주사위 조합 찾기
    fn find_optimal_dice_for_rule(&self, rule: DiceRule) -> ([i32; 5], i32) {
        let my_dice = &self.my_state.dice;
        
        match rule {
            // 기본 점수 규칙: 해당 숫자가 3개 이상이면 선택
            DiceRule::One => {
                let ones: Vec<i32> = my_dice.iter().filter(|&&d| d == 1).cloned().collect();
                if ones.len() >= 3 {
                    let mut dice = [1; 5];
                    for (i, &d) in ones.iter().take(5).enumerate() {
                        dice[i] = d;
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
            ...
            DiceRule::Two => {
                let twos: Vec<i32> = my_dice.iter().filter(|&&d| d == 2).cloned().collect();
                if twos.len() >= 3 {
                    let mut dice = [2; 5];
                    for (i, &d) in twos.iter().take(5).enumerate() {
                        dice[i] = d;
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
            ...
            DiceRule::Three => {
                let threes: Vec<i32> = my_dice.iter().filter(|&&d| d == 3).cloned().collect();
                if threes.len() >= 3 {
                    let mut dice = [3; 5];
                    for (i, &d) in threes.iter().take(5).enumerate() {
                        dice[i] = d;
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
            ...
            DiceRule::Four => {
                let fours: Vec<i32> = my_dice.iter().filter(|&&d| d == 4).cloned().collect();
                if fours.len() >= 3 {
                    let mut dice = [4; 5];
                    for (i, &d) in fours.iter().take(5).enumerate() {
                        dice[i] = d;
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
            ...
            DiceRule::Five => {
                let fives: Vec<i32> = my_dice.iter().filter(|&&d| d == 5).cloned().collect();
                if fives.len() >= 3 {
                    let mut dice = [5; 5];
                    for (i, &d) in fives.iter().take(5).enumerate() {
                        dice[i] = d;
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
            ...
            DiceRule::Six => {
                let sixes: Vec<i32> = my_dice.iter().filter(|&&d| d == 6).cloned().collect();
                if sixes.len() >= 3 {
                    let mut dice = [6; 5];
                    for (i, &d) in sixes.iter().take(5).enumerate() {
                        dice[i] = d;
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
            ...
            DiceRule::Choice => {
                ...
                if self.current_round >= 10 {
                    // 가장 높은 숫자 5개 선택
                    let mut sorted_dice = my_dice.clone();
                    sorted_dice.sort_by(|a, b| b.cmp(a)); // 내림차순 정렬
                    let dice: [i32; 5] = sorted_dice[..5].try_into().unwrap();
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
                } else {
            ...
            DiceRule::FourOfAKind => {
                // 4개 이상 중복된 숫자 찾기
                for num in 1..=6 {
                    let count = my_dice.iter().filter(|&&d| d == num).count();
                    if count >= 4 {
                        let mut dice = [num; 5];
                        // 나머지 1개는 가장 높은 숫자로
                        let remaining: Vec<i32> = my_dice.iter().filter(|&&d| d != num).cloned().collect();
                        if !remaining.is_empty() {
                            dice[4] = *remaining.iter().max().unwrap();
                        }
                        let score = self.calculate_rule_potential_score(&dice, rule);
                        return (dice, score);
                    }
            ...
            DiceRule::FullHouse => {
                // 3개 + 2개 조합 찾기
                for num1 in 1..=6 {
                    let count1 = my_dice.iter().filter(|&&d| d == num1).count();
                    if count1 >= 3 {
                        for num2 in 1..=6 {
                            if num2 != num1 {
                                let count2 = my_dice.iter().filter(|&&d| d == num2).count();
                                if count2 >= 2 {
                                    let mut dice = [num1; 5];
                                    dice[3] = num2;
                                    dice[4] = num2;
            ...
            DiceRule::SmallStraight => {
                ...
                let mut sorted_dice = my_dice.clone();
                sorted_dice.sort();
                sorted_dice.dedup(); // 중복 제거
                
                // 1234 찾기
                if sorted_dice.contains(&1) && sorted_dice.contains(&2) && 
                   sorted_dice.contains(&3) && sorted_dice.contains(&4) {
                    let mut dice = [1, 2, 3, 4, 5];
                    // 5번째는 가장 높은 숫자
                    let remaining: Vec<i32> = my_dice.iter().filter(|&&d| d != 1 && d != 2 && d != 3 && d != 4).cloned().collect();
                    if !remaining.is_empty() {
                        dice[4] = *remaining.iter().max().unwrap();
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                ...
                // 2345 찾기
                if sorted_dice.contains(&2) && sorted_dice.contains(&3) && 
                   sorted_dice.contains(&4) && sorted_dice.contains(&5) {
                    let mut dice = [2, 3, 4, 5, 6];
                    // 6번째는 가장 높은 숫자
                    let remaining: Vec<i32> = my_dice.iter().filter(|&&d| d != 2 && d != 3 && d != 4 && d != 5).cloned().collect();
                    if !remaining.is_empty() {
                        dice[4] = *remaining.iter().max().unwrap();
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
                ...
                // 3456 찾기
                if sorted_dice.contains(&3) && sorted_dice.contains(&4) && 
                   sorted_dice.contains(&5) && sorted_dice.contains(&6) {
                    let mut dice = [3, 4, 5, 6, 1];
                    // 1번째는 가장 높은 숫자
                    let remaining: Vec<i32> = my_dice.iter().filter(|&&d| d != 3 && d != 4 && d != 5 && d != 6).cloned().collect();
                    if !remaining.is_empty() {
                        dice[4] = *remaining.iter().max().unwrap();
                    }
                    let score = self.calculate_rule_potential_score(&dice, rule);
            ...
            DiceRule::LargeStraight => {
                ...
                // 12345 찾기
                if sorted_dice.contains(&1) && sorted_dice.contains(&2) && 
                   sorted_dice.contains(&3) && sorted_dice.contains(&4) && sorted_dice.contains(&5) {
                    let dice = [1, 2, 3, 4, 5];
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
                }
                ...
                // 23456 찾기
                if sorted_dice.contains(&2) && sorted_dice.contains(&3) && 
                   sorted_dice.contains(&4) && sorted_dice.contains(&5) && sorted_dice.contains(&6) {
                    let dice = [2, 3, 4, 5, 6];
                    let score = self.calculate_rule_potential_score(&dice, rule);
                    return (dice, score);
                }
        ...
        let dice: [i32; 5] = if my_dice.len() >= 5 {
            my_dice[..5].try_into().unwrap()
        } else {
            [0; 5]
        };
        let score = self.calculate_rule_potential_score(&dice, rule);
    ...
    fn calculate_13th_round_put(&self) -> DicePut {
        let my_dice = &self.my_state.dice;
        ...
        let dice: [i32; 5] = if my_dice.len() >= 5 {
            ...
            // 주사위가 5개 미만인 경우 (이론적으로는 발생하지 않음)
            let mut fallback_dice = [0; 5];
            for (i, &d) in my_dice.iter().enumerate() {
                if i < 5 {
                    fallback_dice[i] = d;
                }
            }
    ...
    /// 10개 주사위에서 가능한 모든 5개 조합을 생성하는 함수
    fn generate_all_5_combinations(&self, dice: &[i32]) -> Vec<[i32; 5]> {
        ...
        // 간단한 방법: 모든 가능한 5개 조합을 생성
        // 실제로는 모든 조합을 생성하는 것이 비효율적이므로, 
        // 중복이 많은 조합들을 우선적으로 생성
        ...
        // 3. 중간 5개 (10개가 있을 때)
        if dice.len() >= 10 {
            let middle_five: [i32; 5] = dice[2..7].try_into().unwrap();
            combinations.push(middle_five);
        }
        
        // 4. 중복이 많은 숫자들을 포함한 조합들
        let mut counts = [0; 7];
        for &d in dice {
            counts[d as usize] += 1;
        }
        
        // 가장 많이 중복된 숫자들을 우선적으로 포함
        for target_num in 1..=6 {
            if counts[target_num as usize] >= 3 {
                // 해당 숫자를 최대한 포함하는 조합 생성
        ...
        // 6. 모든 숫자가 다른 조합들 (CHOICE용)
        let mut unique_combination = [0; 5];
        let mut used = 0;
        let mut used_numbers = [false; 7];
        
        for &d in dice {
            if !used_numbers[d as usize] && used < 5 {
                unique_combination[used] = d;
                used_numbers[d as usize] = true;
                used += 1;
            }
        }
        
        if used == 5 {
            combinations.push(unique_combination);
        }
        
        // 샘플링 적용: 조합 수가 너무 많으면 제한
        if combinations.len() > MAX_COMBINATIONS {
            // 전략적으로 중요한 조합들을 우선적으로 유지
            let mut sampled_combinations = Vec::new();
            
            // 1. 처음 5개, 마지막 5개는 항상 포함
            if combinations.len() >= 2 {
                sampled_combinations.push(combinations[0]);
                sampled_combinations.push(combinations[1]);
            }
            
            // 2. 중복이 많은 조합들을 우선적으로 포함
            for i in 2..combinations.len().min(MAX_COMBINATIONS - 10) {
                sampled_combinations.push(combinations[i]);
            }
            
            // 3. 나머지는 균등하게 샘플링
            let step = combinations.len() / (MAX_COMBINATIONS - sampled_combinations.len());
            for i in (MAX_COMBINATIONS - 10..combinations.len()).step_by(step.max(1)) {
                if sampled_combinations.len() < MAX_COMBINATIONS {
                    sampled_combinations.push(combinations[i]);
                }
            }
    ...
    fn find_best_dice_for_rule(&self, rule: DiceRule) -> ([i32; 5], i32) {
        ...
        if my_dice.len() >= 5 {
            // 중복 개수에 따른 전략적 조합 생성
            let combinations = self.generate_strategic_combinations(my_dice);
    ...
    /// 중복 개수에 따른 전략적 조합을 생성하는 함수
    fn generate_strategic_combinations(&self, dice: &[i32]) -> Vec<[i32; 5]> {
        let mut combinations = Vec::new();
        ...
        // 8번째 턴 이후 특별 전략 적용
        if self.current_round >= 8 {
            return self.generate_late_game_combinations(dice);
        }
        
        // 내가 보유한 주사위의 중복 개수 분석
        let mut counts = [0; 7];  // 1~6까지의 개수 (인덱스 0은 사용하지 않음)
        for &d in dice {
            counts[d as usize] += 1;
        }
        ...
        // 중복 개수에 따른 전략적 조합 생성
        if max_count >= 4 {
            // 4개 이상 중복: YACHT나 FOUR_OF_A_KIND를 위해 아껴두기
            if self.my_yacht_completed {
                // YACHT를 이미 달성했다면 FOUR_OF_A_KIND만 고려
                combinations.extend(self.generate_conservative_combinations(dice, max_number as i32));
            } else {
                // YACHT를 아직 달성하지 않았다면 YACHT도 고려
                combinations.extend(self.generate_conservative_combinations(dice, max_number as i32));
            }
        } else if max_count >= 3 {
            // 3개 중복: FULL_HOUSE나 FOUR_OF_A_KIND를 위해 아껴두기
            combinations.extend(self.generate_conservative_combinations(dice, max_number as i32));
        } else if max_count >= 2 {
            // 2개 중복: 조심스럽게 사용
            combinations.extend(self.generate_moderate_combinations(dice, max_number as i32));
        } else {
            // 중복 없음: 자유롭게 사용
            combinations.extend(self.generate_aggressive_combinations(dice));
        }
    ...
    /// 8번째 턴 이후의 후반 게임 조합 생성 전략
    fn generate_late_game_combinations(&self, dice: &[i32]) -> Vec<[i32; 5]> {
        let mut combinations = Vec::new();
        ...
        // SMALL_STRAIGHT나 LARGE_STRAIGHT가 남아있는지 확인
        let has_small_straight = unused_rules.contains(&(DiceRule::SmallStraight as usize));
        let has_large_straight = unused_rules.contains(&(DiceRule::LargeStraight as usize));
        
        if has_small_straight || has_large_straight {
            // STRAIGHT를 위해 노력하는 조합들 생성
            combinations.extend(self.generate_straight_focused_combinations(dice));
        } else {
            // STRAIGHT를 이미 달성했으므로 높은 숫자와 중복 우선
            combinations.extend(self.generate_high_value_combinations(dice));
        }
        
        combinations
    }
    
    /// STRAIGHT 달성을 위한 조합 생성
    fn generate_straight_focused_combinations(&self, dice: &[i32]) -> Vec<[i32; 5]> {
        let mut combinations = Vec::new();
        
        // 1. 연속된 숫자 조합들 (STRAIGHT용)
        for start in 1..=2 {
            let mut combination = [0; 5];
            let mut used = 0;
        ...
        // 2. 실제 주사위에서 연속된 부분 찾기
        let mut sorted_dice = dice.to_vec();
        sorted_dice.sort();
        
        // 연속된 4개나 5개 찾기
        for i in 0..=sorted_dice.len().saturating_sub(4) {
            let mut consecutive = Vec::new();
            let mut current = sorted_dice[i];
            consecutive.push(current);
            
            for j in i+1..sorted_dice.len() {
                if sorted_dice[j] == current + 1 {
                    consecutive.push(sorted_dice[j]);
                    current = sorted_dice[j];
                }
            }
            
            if consecutive.len() >= 4 {
                // 연속된 숫자들을 포함하는 조합 생성
                let mut combination = [0; 5];
                let mut used = 0;
                ...
                // 나머지는 다른 숫자들로 채우기
                for &d in dice {
                    if !consecutive.contains(&d) && used < 5 {
                        combination[used] = d;
                        used += 1;
                    }
                }
    ...
    /// 높은 숫자와 중복을 우선하는 조합 생성
    fn generate_high_value_combinations(&self, dice: &[i32]) -> Vec<[i32; 5]> {
        let mut combinations = Vec::new();
        ...
        // 가장 많이 중복된 숫자들을 우선적으로 포함
        for target_num in (1..=6).rev() {  // 높은 숫자부터
            if counts[target_num as usize] >= 2 {
                // 해당 숫자를 최대한 포함하는 조합 생성
                let mut combination = [0; 5];
                let mut used = 0;
        ...
        // 높은 숫자 우선 조합
        let mut high_value_combination = [0; 5];
        let mut used = 0;
        ...
        if used == 5 {
            combinations.push(high_value_combination);
        }
    ...
    /// 보수적인 조합 생성 (중복을 아껴두기)
    fn generate_conservative_combinations(&self, dice: &[i32], preserve_number: i32) -> Vec<[i32; 5]> {
        let mut combinations = Vec::new();
        
        if self.my_yacht_completed {
            // YACHT를 이미 달성했다면 더 적극적으로 사용
            // 중복이 많은 숫자를 2개까지 사용
            let mut combination = [0; 5];
            let mut used = 0;
            ...
            // YACHT를 아직 달성하지 않았다면 보수적으로 사용
            // 중복이 많은 숫자는 최대 1개만 사용
            let mut combination = [0; 5];
            let mut used = 0;
            
            // 중복이 많은 숫자는 1개만 사용
            for &d in dice {
                if d == preserve_number && used < 1 {
                    combination[used] = d;
                    used += 1;
                }
            }
    ...
    /// 중간 정도의 조합 생성 (2개 중복)
    fn generate_moderate_combinations(&self, dice: &[i32], duplicate_number: i32) -> Vec<[i32; 5]> {
        let mut combinations = Vec::new();
    ...
    /// 공격적인 조합 생성 (중복 없음)
    fn generate_aggressive_combinations(&self, dice: &[i32]) -> Vec<[i32; 5]> {
        let mut combinations = Vec::new();
