...
class Game:
    ...
    def calculate_strategic_bid_amount(self, dice_a: List[int], dice_b: List[int], selected_group: str) -> int:
        ...
        if self.current_round <= 2:
            ...
            if combined_max_count == 3:
                ...
                # 중복수가 4개인 카드가 나오면 (최대 입찰 - 1) 선택
                return max_opponent_bid - 1
            else:
                # 그 외의 경우 0 입찰
                return 0
    ...
    def should_use_rule_strategically(self, rule: DiceRule, score: int) -> bool:
        ...
        if rule == DiceRule.SMALL_STRAIGHT:
            if self.current_round <= 8:  # 초반에는 아껴두기
                return score > 0 and self.current_round >= 6
            else:  # 후반에는 사용
                return score > 0
    ...
    def find_optimal_dice_for_rule(self, rule: DiceRule) -> Tuple[List[int], int]:
        ...
        if rule == DiceRule.ONE:
            ones = [d for d in my_dice if d == 1]
            if len(ones) >= 3:
                # 실제 보유한 1들을 사용하고, 부족하면 다른 주사위로 채움
                dice = []
                # 1을 최대 5개까지 사용
                for i in range(min(5, len(ones))):
                    dice.append(ones[i])
                    my_dice.remove(ones[i])  # 사용한 주사위 제거
                
                # 5개가 되도록 다른 주사위로 채움
                while len(dice) < 5 and my_dice:
                    dice.append(my_dice.pop(0))
            ...
            twos = [d for d in my_dice if d == 2]
            if len(twos) >= 3:
                dice = []
                for i in range(min(5, len(twos))):
                    dice.append(twos[i])
                    my_dice.remove(twos[i])
            ...
            threes = [d for d in my_dice if d == 3]
            if len(threes) >= 3:
                dice = []
                for i in range(min(5, len(threes))):
                    dice.append(threes[i])
                    my_dice.remove(threes[i])
            ...
            fours = [d for d in my_dice if d == 4]
            if len(fours) >= 3:
                dice = []
                for i in range(min(5, len(fours))):
                    dice.append(fours[i])
                    my_dice.remove(fours[i])
            ...
            fives = [d for d in my_dice if d == 5]
            if len(fives) >= 3:
                dice = []
                for i in range(min(5, len(fives))):
                    dice.append(fives[i])
                    my_dice.remove(fives[i])
            ...
            sixes = [d for d in my_dice if d == 6]
            if len(sixes) >= 3:
                dice = []
                for i in range(min(5, len(sixes))):
                    dice.append(sixes[i])
                    my_dice.remove(sixes[i])
            ...
            if self.current_round >= 10:
                # 가장 높은 숫자 5개 선택
                sorted_dice = sorted(my_dice, reverse=True)
                dice = sorted_dice[:5]
                score = self.calculate_rule_potential_score(dice, rule)
                return (dice, score)
            else:
            ...
            sorted_dice = sorted(set(my_dice))
            
            # 1234 찾기
            if all(x in sorted_dice for x in [1, 2, 3, 4]):
                # 실제 보유한 주사위들로 조합 생성
                target_dice = []
                temp_dice = my_dice.copy()
                ...
                if temp_dice:
                    ...
                    target_dice.append(5)  # 기본값
                
                score = self.calculate_rule_potential_score(target_dice, rule)
                return (target_dice, score)
            
            # 2345 찾기
            elif all(x in sorted_dice for x in [2, 3, 4, 5]):
                target_dice = []
                temp_dice = my_dice.copy()
                ...
                if temp_dice:
                    ...
                    target_dice.append(6)
                
                score = self.calculate_rule_potential_score(target_dice, rule)
                return (target_dice, score)
            
            # 3456 찾기
            elif all(x in sorted_dice for x in [3, 4, 5, 6]):
                target_dice = []
                temp_dice = my_dice.copy()
